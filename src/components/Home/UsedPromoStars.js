...
class UsedPromoStars extends Component{
  state = {customers: [],valueC:'',results:[], showResults:false,notFound:false,}
  handleSubmit = () => {
    axios.get(`http://localhost:3001/api/promostarUsed/${this.state.valueC}`)
    .then((response) => {
  ...
  componentDidMount(){
    ...
    return(
        <div>
          ...
          <Modal dimmer trigger={<Button color='orange' inverted>View Promo Stars Used by a Customer</Button>} >
            <Modal.Header>View All Valid Promo Stars Used By A Customer</Modal.Header>
            <Modal.Content >

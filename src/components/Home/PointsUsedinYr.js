...
class PointsUsedInYr extends Component{
  state = {customers: [],valueC:'',Year:'',results:[], showResults:false,notFound:false}
  handleSubmit = () => {
    axios.get(`http://localhost:3001/api/rewardUsedInYr/${this.state.valueC}/${this.state.Year}`)
    .then((response) => {
  ...
  componentDidMount(){
    ...
    return(
        <div>
          ...
          <Modal dimmer trigger={<Button color='orange' inverted>View Points Used by a Customer in a Year</Button>} >
            <Modal.Header>View All Points Used By A Customer In Year</Modal.Header>
            <Modal.Content >

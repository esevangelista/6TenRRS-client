...
class PointsEarnedInYr extends Component{
  state = {customers: [],valueC:'',Year:'',results:[], showResults:false,notFound:false}
  handleSubmit = () => {
    axios.get(`http://localhost:3001/api/rewardInYr/${this.state.valueC}/${this.state.Year}`)
    .then((response) => {
  ...
  handleChange = (e,{value}) => this.setState({Year:value})
  ...
  componentDidMount(){
	...
	render(){
    const {customers,value,results,showResults,notFound,Year} = this.state;
    return(
        <div>
           { showResults && 
            ...
            (
                <Modal open={notFound} onClose={this.closeMessage} basic blurring='true'>
                    <Message
                        warning
                        header='Not Found'
                        content='The customer rewards in the specified year cannot be found'
                    />
                </Modal>
            )
          }
          <Modal dimmer trigger={<Button color='orange' inverted>View Points Earned by a Customer in a Year</Button>} >
            <Modal.Header>View All Points Earned By A Customer In Year</Modal.Header>
            <Modal.Content >
                ...
                <Form.Input  label='Year' name='Year' placeholder='YYYY' value={this.state.Year} onChange={this.handleChange} required/>
...
export default PointsEarnedInYr;
